...
CONTAINER_ID=$(cat /dev/urandom | tr -dc 'a-z0-9A-Z' | fold -w 8 | head -n 1)
DOCKER_DATA="/digis_data"
DOCKER_CONTAINER_NAME="digis_${CONTAINER_ID}"
echo $DOCKER_CONTAINER_NAME
DOCKER_RUN="sudo docker run --name ${DOCKER_CONTAINER_NAME}"
DOCKER_IMAGE_NAME="janka2012/digis"
...
then
	...
	DOCKER_PARAMS="${DOCKER_PARANS} -i ${DOCKER_DATA}/$(basename $INPUT_FASTA)"
else
...
DOCKER_PARAMS="${DOCKER_PARAMS} -o ${DOCKER_OUTPUTDIR}"
